...
import { ATTR_NAMES, ATTR_IMAGES, ATTR_COLORS } from '@/lib/constants'
...
export const AttributeDetail = memo(function AttributeDetail({ attrIdx, onSelect }: Props) {
  ...
  useEffect(() => { setSortKey(k => k + 1); setMinAtk(0) }, [attrIdx])
  ...
  const sorted = useMemo(() =>
    minAtk > 0 ? base.filter(c => c.Attack >= minAtk) : base,
    [base, minAtk]
  )
  ...
  return (
    <div className="pb-8">
      <div className="flex items-center gap-3 px-4 py-4 border-b border-[#151520]">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[10px] text-[#555] uppercase tracking-widest mt-0.5">
            {sorted.length}{minAtk > 0 ? ` / ${base.length}` : ''} cards
          </div>
        </div>
      ...
      <CardListView cards={sorted} sortKey={sortKey} accentColor={color} onSelect={onSelect} onViewModeChange={setViewMode} />
    </div>
  )
})
